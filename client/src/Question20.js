...
export default function Question20() {
  ...
  useEffect(() => {
    ...
    var config = {
      method: "get",
      url: "https://api.fastforex.io/fetch-all?api_key=ebfa47f4c1-844e0441cd-r713e3&from=USD&to=RUB",
      headers: {},
    };

    axios(config)
      .then(function (response) {
        console.log(response.data.results.RUB);
        setRate(response.data.results.RUB);
      })
    ...
    <div className="main">
      <div className="sticky-sub-div">
        <h2 className="percent">
          {Math.round(((100 / 29) * 19).toString())}% завершено
        </h2>
        ...
        <div className="left-align-text">
          <h4>Секция: Курсы валют</h4>
          <p className="question">
            <strong>
              <span style={{ color: "#db536a" }}>USD/RUB</span>
            </strong>{" "}
            - Какие значения курса доллара США по отношению к российскому рублю,
            по Вашему мнению, являются наиболее вероятными в перспективе
            следующих периодов?
          </p>
